...
/// Loopback relay URL used only by the legacy local-profile fallback and tests.
...
pub struct MessagingStore {
    ...
    /// Identity public keys (base64) of blocked contacts. Messages from these
    /// identities are ack-and-discarded on inbound, and auto-contact creation
...
fn normalize_server_url(url: Option<String>) -> String {
    let raw = url.unwrap_or_default().trim().to_string();
...
fn is_acceptable_relay_url(url: &str) -> bool {
    ...
    match ws_url_host(&normalized) {
        Some(host) => host.ends_with(".onion") || host == "127.0.0.1" || host == "localhost",
        None => false,
    }
}
pub fn server_id_for_url(url: &str) -> String { format!("srv_{}", hex::encode(&Sha256::digest(url.as_bytes())[..16])) }
...
#[derive(Debug, Clone, Serialize)]
pub struct InspectedConnectionCode {
    pub server_url: String,
    pub server_name: String,
...
/// through, so the UI can warn before the user commits to a slow Tor add. The
/// server URL is carried in the code itself for both direct and hosted invites.
pub async fn inspect_connection_code(
    app: AppHandle,
    session: &AppSessionState,
    code: String,
) -> Result<InspectedConnectionCode, String> {
    let server_url = match decode_invite_code(&code)? {
        DecodedInviteCode::Direct(payload) => payload.server_url,
        DecodedInviteCode::Hosted(hosted) => hosted.server_url,
    };
    let normalized = normalize_server_url(Some(server_url));
    let (store_key, legacy_store_key) = store_keys(session).await?;
    let store = load_store_with_keys(&app, &store_key, &legacy_store_key)?;
    let is_known_private = store
        .private_servers
        .iter()
...
pub async fn save_private_server_settings(app: AppHandle, session: &AppSessionState, settings: PrivateServerSettings) -> Result<PrivateServerSettings, String> {
    ...
    for server in settings.private_servers.into_iter().take(32) {
        ...
        if !is_acceptable_relay_url(&onion) {
            return Err("relays must be ws:// or wss:// URLs with a .onion or loopback host".to_string());
        }
...
pub async fn generate_connection_code(
    ...
    let (response, route_for_connect, bundle_id, hosted_ciphertext, expires) = {
        ...
        if server_url.is_empty() {
            return Err("no relay selected; add a relay in Settings and choose it as your default before generating a connection code".to_string());
        }
...
/// Delete a conversation. Always removes the local contact, its message history,
/// its Signal sessions, and its per-contact receive routes. When `retire_relay`
/// is set, the relay mailboxes backing those routes are also torn down on the
/// relay (best-effort, in the background, since each retire can take 30–150s over
/// Tor); otherwise the relay side is left untouched and simply ages out on its own.
pub async fn delete_contact(
    ...
    let contact_routes: Vec<LocalRoute> = {
        let _store_guard = session.messaging_store_lock.lock().await;
        let mut store = load_store_with_keys(&app, &store_key, &legacy_store_key)?;
        let contact = store.contacts.iter().find(|c| c.id == contact_id).cloned()
            .ok_or_else(|| "contact not found".to_string())?;

        let belongs_to_contact = |route: &LocalRoute| {
            route.scope == contact_route_scope(&contact_id)
                || route.scope == format!("sender:{contact_id}")
                || route.scope == format!("retiring_invite:{contact_id}")
        };
        let contact_routes: Vec<LocalRoute> =
    ...
    if retire_relay && !contact_routes.is_empty() {
        ...
        tokio::spawn(async move {
            let mut retired_targets: Vec<(String, String)> = Vec::new();
            for route in &contact_routes {
                ...
                if retired {
                    retired_targets.push((route.server_url.clone(), route.mailbox_id.clone()));
                }
            }
            ...
            if let Ok(mut store) = load_store_with_keys(&app_retire, &store_key, &legacy_store_key) {
                // Clear only the entries we successfully retired. Any that failed
                // stay queued so connect_all retries them, and unrelated pending
                // retirements are left intact.
                store.pending_relay_retires.retain(|pending| {
                    !retired_targets.iter().any(|(url, mailbox)| url == &pending.server_url && mailbox == &pending.mailbox_id)
                });
...
pub struct BlockContactResponse {
    /// Number of active self-generated connection codes on the same relay(s)
    /// as the blocked contact. If > 0 the UI should warn: "they may still hold
    /// a code you created; delete it to stop them reaching your relay at all
    /// (this disconnects anyone else you gave it to)."
    pub active_code_count: usize,
    /// IDs of the active codes, so the UI can offer one-click deletion.
    pub active_code_ids: Vec<String>,
}

/// Delete a conversation **and** permanently block the contact's identity.
///
/// Three layers:
/// 1. Record the contact's `identity_public_b64` in `blocked_identities` so
///    any future inbound message from that identity is ack-and-discarded.
/// 2. Retire all mailboxes used for this contact (immediate cut).
/// 3. Return info about active self-generated connection codes on the same
///    relay so the UI can warn/offer to delete them.
pub async fn delete_and_block_contact(
    app: AppHandle,
    session: &AppSessionState,
    transport_state: &transport::TransportState,
    tor_client: Arc<Mutex<Option<arti_client::TorClient<tor_rtcompat::PreferredRuntime>>>>,
    contact_id: String,
) -> Result<BlockContactResponse, String> {
    let (store_key, legacy_store_key) = store_keys(session).await?;

    // Gather the identity key and relay info before deleting.
    let (identity_b64, contact_server_ids): (String, HashSet<String>) = {
        let _store_guard = session.messaging_store_lock.lock().await;
        let store = load_store_with_keys(&app, &store_key, &legacy_store_key)?;
        let contact = store.contacts.iter().find(|c| c.id == contact_id).cloned()
            .ok_or_else(|| "contact not found".to_string())?;
        let identity = contact.identity_public_b64.clone();
        // Collect all relay server IDs this contact has routes on.
        let mut server_ids: HashSet<String> = HashSet::new();
        if !contact.server_id.is_empty() { server_ids.insert(contact.server_id.clone()); }
        for route in store.local_routes.iter().filter(|r| {
            r.scope == contact_route_scope(&contact_id)
                || r.scope == format!("sender:{contact_id}")
                || r.scope == format!("retiring_invite:{contact_id}")
        }) {
            if !route.server_id.is_empty() { server_ids.insert(route.server_id.clone()); }
        }
        (identity, server_ids)
    };

    // Layer 1: record identity in blocklist.
    if !identity_b64.trim().is_empty() {
        let _store_guard = session.messaging_store_lock.lock().await;
        let mut store = load_store_with_keys(&app, &store_key, &legacy_store_key)?;
        store.blocked_identities.insert(identity_b64.clone());
        save_store_with_key(&app, &store, &store_key)?;
    }

    // Layer 2: delete contact + retire mailboxes (reuses existing logic).
    delete_contact(
        app.clone(), session, transport_state, tor_client, contact_id.clone(), true,
    ).await?;

    // Layer 3: check for active self-generated codes on the same relay(s).
    let (active_code_count, active_code_ids) = {
        let _store_guard = session.messaging_store_lock.lock().await;
        let store = load_store_with_keys(&app, &store_key, &legacy_store_key)?;
        let now = now_ms();
        let codes: Vec<String> = store.pending_invites.iter()
            .filter(|invite| {
                invite.expires_at_ms > now && {
                    let invite_server_id = server_id_for_url(&invite.server_url);
                    contact_server_ids.contains(&invite_server_id)
                }
            })
    ...
    Ok(BlockContactResponse { active_code_count, active_code_ids })
}
...
pub async fn add_contact_from_code(
    ...
    // Refuse to add a contact whose identity has been blocked.
    if !contact.identity_public_b64.is_empty()
        && store.blocked_identities.contains(&contact.identity_public_b64)
    {
...
async fn send_signal_payload_internal(
    ...
    let cert = match transport::request_sender_certificate(
        ...
        Err(primary_err) => {
            ...
            // Do NOT force-disconnect here. The primary cert request typically
            // failed only because the long-lived route was still building its Tor
            // circuit (a >10s onion connect), not because it was dead. Tearing it
            // down would race a second run_connection against the one that is
            // still establishing, and both would fight over the relay's `online`
            // slot for this mailbox, flapping inbound delivery. connect_server is
            // idempotent: it reuses a healthy connection and only rebuilds one
            // whose outbound channel has closed (writer dead). The keepalive Ping
            // ensures a wedged circuit closes that channel promptly, so a genuine
            // dead connection still gets rebuilt here.
            let _ = transport::connect_server(
                app.clone(),
                transport_state,
                tor_client.clone(),
    ...
    // Prefer the warm, already-established connection (the same one that just
    // issued the sender certificate) so the send rides an existing Tor circuit
    // instead of dialling a brand-new onion circuit per message. Building a
    // fresh circuit for every send is the root cause of the Tor-only failures
    // after a restart: with Arti's circuit and hidden-service descriptor caches
    // cold, those one-shot dials are slow and flaky, which is what surfaces as
    // messages hanging on "sending" or stalling at "pending". On localhost the
    // one-shot connect is instant and reliable, so the bug never shows there.
    //
    // Fall back to a one-shot fresh circuit only when no live connection is
    // available (e.g. the certificate came back over the short-lived fallback
    // socket and the long-lived route is still reconnecting). The relay assigns
    // each SendEnvelope a fresh id, and libsignal rejects a replayed ciphertext,
    // so a fallback retry cannot surface a duplicate message to the peer.
    let send_result = match transport::send_envelope(
        transport_state,
        connection_id_for_cert.clone(),
        send_to.clone(),
        send_delivery_token.clone(),
        ENVELOPE_TYPE_SEALED_SIGNAL.to_string(),
        wire_json.clone(),
        Some(message_id.clone()),
    ).await {
        Ok(ack) => Ok(ack),
        Err(_) => transport::send_envelope_once(
            tor_client.clone(),
            send_server_url,
            send_to,
            send_delivery_token,
...
pub async fn handle_incoming_envelope(
    ...
    let local_route = match route_for_mailbox(&store, &envelope.to) {
        ...
        None => match store.local_profile.clone() {
            Some(profile) => legacy_route_from_profile(profile, server_id.clone(), LOOPBACK_RELAY_URL.to_string())?,
    ...
    // ── Layer 1 enforcement: identity blocklist ──
    // If the now-revealed sender identity is blocked, ack-and-discard. Also undo
    // any auto-created contact that decrypt_sealed_sender_message may have pushed
    // into the store so the conversation never appears in the UI.
    if !decrypted.contact.identity_public_b64.is_empty()
        && store.blocked_identities.contains(&decrypted.contact.identity_public_b64)
    {
        // Remove the auto-created contact (if it was just pushed during decrypt)
        // and any Signal session decrypt persisted for this identity, so a blocked
        // sender cannot leave durable ratchet state behind in the store.
        let blocked_identity = decrypted.contact.identity_public_b64.clone();
        store.contacts.retain(|c| c.identity_public_b64 != blocked_identity);
        store.signal_sessions.retain(|_, s| s.remote_identity_b64 != blocked_identity);
        save_store_with_key(&app, &store, &store_key)?;
        drop(store);
        drop(_store_guard);
        let _ = transport::ack_envelopes(transport_state, server_id.clone(), vec![envelope.id]).await;
