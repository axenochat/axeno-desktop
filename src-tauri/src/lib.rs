...
#[tauri::command]
async fn messaging_inspect_connection_code(
    app: AppHandle,
    session: State<'_, AppSessionState>,
    code: String,
) -> Result<messaging::InspectedConnectionCode, String> {
    messaging::inspect_connection_code(app, &session, code).await
}

#[tauri::command]
async fn messaging_delete_contact(
    app: AppHandle,
    session: State<'_, AppSessionState>,
    transport_state: State<'_, transport::TransportState>,
    tor_state: State<'_, AppTorState>,
    ...
    messaging::delete_contact(app, &session, transport_state.inner(), tor_state.client.clone(), contact_id, retire_relay).await
}

#[tauri::command]
async fn messaging_delete_and_block_contact(
    app: AppHandle,
    session: State<'_, AppSessionState>,
    transport_state: State<'_, transport::TransportState>,
    tor_state: State<'_, AppTorState>,
    contact_id: String,
) -> Result<messaging::BlockContactResponse, String> {
    messaging::delete_and_block_contact(app, &session, transport_state.inner(), tor_state.client.clone(), contact_id).await
}
...
pub fn run() {
    // WebKitGTK on some Linux GPU/driver combinations renders a blank or
    // corrupted window unless hardware compositing is disabled. This must be set
    // before the webview is created. Doing it here (rather than as a shell prefix
    // in package.json) keeps it Linux-only — the env-var prefix syntax breaks on
    // Windows — and also applies to the shipped app and CI builds, not just dev.
    #[cfg(target_os = "linux")]
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // frontend drives the check/prompt/install flow (see useUpdater).
            #[cfg(desktop)]
            app.handle()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            messaging_inspect_connection_code,
            messaging_delete_contact,
