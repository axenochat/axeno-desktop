...
fn verify_unlocked_consistency(blob: &EncryptedIdentity, secrets: &VaultSecrets) -> Result<(), IdentityError> {
    ...
    if !identity_pub.verify_signature(&blob.signed_prekey_public, &blob.signed_prekey_signature) {
        return Err(IdentityError::Signal("signed prekey signature does not match identity key".to_string()));
    }
